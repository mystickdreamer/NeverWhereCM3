...
void do_piss( CHAR_DATA * ch, char *argument )
{
   ...
   if( ( part = find_bodypart( ch, BP_GROIN ) ) != NULL )
   {
      if( part->obj )
      {
   ...
   else if( ( part = find_bodypart( ch, BP_GROIN ) ) != NULL )
   {
      if( part->obj )
      {
         act( AT_YELLOW, "You piss your pants.", ch, NULL, NULL, TO_CHAR );
   ...
   if( victim && ( can_use_bodypart( ch, BP_GROIN ) || IS_AFFECTED( ch, AFF_FLYING ) || victim->position < POS_SQUATTING ) )
   {
