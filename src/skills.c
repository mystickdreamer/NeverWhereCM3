...
//Reworking skinning to try and use materials that can be used to craft
      for( material = first_material; material; material = material->next )
      {
         if( ( corpse->material->race == material->race ) )
         {
            ...
            if( obj )
	  {
	     ...
	     {
	     send_to_char( "Sorry this can't be skinned.\n\r", ch);
	     }
         }
      }
